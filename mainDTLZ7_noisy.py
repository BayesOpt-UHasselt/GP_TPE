...
base_configuration  = {
    ...
    'k': k,
    'seed': seed}
benchmark_name      = "DTLZ7" #args.benchmark_name
...
mmm.to_csv("output/replications_noisy_MOTPE_DTLZ7.csv")

# Save the exploration
m = solver.get_F()
df = pd.DataFrame(m)
df.to_csv("output/exploration_noisy_MOTPE_DTLZ7.csv")

m = solver.get_F()
pf = pareto_front(m, index=True)
pf = m[pf]
volume=compute_hypervolume(pf, [1, 23])
...
plt.savefig('output/PF_noisy_MOTPE_DTLZ7.png', dpi=1200, bbox_inches='tight')
...
plt.savefig('output/HV_noisy_MOTPE_DTLZ7.png', dpi=1200, bbox_inches='tight')

# Save the hyervolume history
df = pd.DataFrame(hv_temp)
df.to_csv("output/hypervolume_noisy_MOTPE_DTLZ7.csv")
