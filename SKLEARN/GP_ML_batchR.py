...
for id_d in datasets_id: # For each dataset
    for id_problem in range(len(variables_array)):
        ...
        try:
            ...
            df = pd.DataFrame(m)
            df.to_csv(
                "../output/GP_sklearn/meanObjectives_GP_val_" + benchmark_name + "_" + str(id_d) + ".csv")

            # Save the evaluation on test set for all the solutions found
            m = solver.test_evaluations()
            df = pd.DataFrame(m)
            df.to_csv(
                "../output/GP_sklearn/meanObjectives_GP_test_" + benchmark_name + "_" + str(id_d) + ".csv")

            rep = solver.history_replications()
            mmm = pd.DataFrame(rep)
            mmm.to_csv("../output/GP_sklearn/replications_GP_" + benchmark_name + "_" + str(id_d) + ".csv")
