...
class HyperVolume:
        ...
        for point in front:
            ...
            if weaklyDominates(point, referencePoint):
                if len(relevantPoints)==0:
                    relevantPoints = np.asarray([point])
                else:
                    relevantPoints = np.append(relevantPoints, np.asarray([point]), axis=0)
        if any(referencePoint):
            # shift points so that referencePoint == [0, ..., 0]
            # this way the reference point doesn't have to be explicitly used
            # in the HV computation
            for j in range(relevantPoints.shape[0]):
    ...
    def sortByDimension(self, nodes, i):
        ...
        just_element = [node.cargo[i] for node in nodes]
        je_indices = np.argsort(just_element)
        # sort by this value
        # decorated.sort()
        # write back to original list
        nodes[:] = [nodes[index] for index in je_indices]
