function [f, mse, DT_MSE] = SKpredict_new(model,Xpred,Bpred, seed)

%Edited by Sebastian, June 20, 2018:
%   * Calculates both SK mse (mse) and DK mse (DT_MSE).
%   * Matrix inversions are done with the '\' operator instead of 'inv'
%     for improving precision.

% This code yields the predictions at prediction points using a stochastic kriging model  
...
% 
% Exmaples
%      SK_gau  = SKpredict_new(skriging_model,XK,ones(K,1));
% Based on parameter estimates skriging_model obtained from SKfit_new.m,
% use SK model to predict the values at prediction points XK with constant
% prediction trend, Bpred = ones(K,1)

% retrieve model parameters from model structure obtained from SKfit_new
rng(seed);
...
% calculate predicted Stochastic kriging MSE at prediction points
B = model.B;
...
% calculate predicted Deterministic kriging MSE at prediction points
DT_MSE=zeros(K,1);

for ci=1:K
 delta= 1-ones(k,1)'* sigmaDTinv * Rpred(:,ci);
 DT_MSE(ci)= tau2- Rpred(:,ci)' * sigmaDTinv * Rpred(:,ci) + delta^2/(ones(k,1)'* sigmaDTinv * ones(k,1));
end
