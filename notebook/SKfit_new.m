function model = SKfit_new(X, Y, B, Vhat, gammaP, algor_sel, seed)

%-----------------------------------------------------------------------------
%% Change log by Sebastian, July 7, 2018:
%    * Upper bounds (Ub) were added for theta

%    * Added 3 optimization methods through the 'algor_sel' input parameter:
%       - algor_sel = 1: trust-region-reflective algorithm
%       - algor_sel = 2: active-set algorithm
%       - algor_sel ~= 1 or 2: interior-point algorithm
%       Note: I reccommend interior-point

%     * Added Matern 2.5 kernel 

%     * Updated codes:
%       SKfit_new.m - calculate Maximum Likelihood Estimates of SK model parameters
%       logPL_new.m - evaluate the objective function, negative of profile log-likelihood  
%       SKpredict_new.m - return SK predictions
...
% 
% Examples
%       skriging_model = SKfit_new(X,Y,ones(k,1),Vhat,2,3);
...
if ((gammaP ~= 1)&&(gammaP ~= 2)&&(gammaP ~= 3)&&(gammaP ~= 4))
    error('please type of correlation function: 1 = exponential, 2 = gauss, 3 = cubic, 4 = Matern 2.5.')
end
...
% see stochastic kriging paper for explanation
...
myopt = optimset('Display','none','MaxFunEvals',1000000,'MaxIter',500,...
    'Algorithm', algor);
parms = fmincon(@(x) logPL_new(x,k,d,D,B,V,Y,gammaP,seed),...
...
% MLE of beta is known in closed form given values for tau2, theta and is computed below
