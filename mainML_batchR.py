...
variables_array = [5]
ML_names = ["DTr"] #"SVMr" , "MLPr"
datasets_id = [189, 507, 504, 529, 42636, 550, 23516]
...
for id in datasets_id: # For each dataset
    for v_index in range(len(variables_array)):
        ...
        target_scaler.fit(np.reshape(y.values, (len(y), 1)))
        y = pd.Series(target_scaler.transform(np.reshape(y.values, (len(y), 1)))[:,0])
        ...
        m = solver.get_F()
        df = pd.DataFrame(m)
        df.to_csv("output/temp/exploration_MOTPE_val_"+ML_algorithm+"_"+str(id)+".csv")

        # Save the evaluation on test set for all the solutions found
        m = solver.test_evaluations()
        df = pd.DataFrame(m)
        df.to_csv("output/temp/exploration_MOTPE_test_"+ML_algorithm+"_"+str(id)+".csv")

        rep = solver.history_replications()
        mmm = pd.DataFrame(rep.reshape(rep.shape[0]*rep.shape[1],2))
        mmm.to_csv("output/temp/replications_MOTPE_"+ML_algorithm+"_"+str(id)+".csv")
