...
replications        = 50
k                   = 1     #args.k
num_candidates      = 5000    #args.num_candidates
...
base_configuration  = {
    ...
    'k': k,
    'seed': seed}
benchmark_name      = "DTLZ6" #args.benchmark_name
...
range_objectives = np.asarray([[5, 5],
                              [0, 0]])
...
# mmm.to_csv("output/replications_noisy_MOTPE_DTLZ6.csv")
#
# # Save the exploration
# m = solver.get_F()
# df = pd.DataFrame(m)
# df.to_csv("output/exploration_noisy_MOTPE_DTLZ6.csv")

m = solver.get_F()
pf = pareto_front(m, index=True)
pf = m[pf]
volume=compute_hypervolume(pf, [10, 10])
...
# plt.savefig('output/PF_noisy_MOTPE_DTLZ6.png', dpi=1200, bbox_inches='tight')
...
# plt.savefig('output/HV_noisy_MOTPE_DTLZ6.png', dpi=1200, bbox_inches='tight')

# Save the hyervolume history
df = pd.DataFrame(hv_temp)
# df.to_csv("output/hypervolume_noisy_MOTPE_DTLZ6.csv")
